...
func NewWorker(logging *log.WatermillLogrusAdapter, publisher *redisstream.Publisher, router *message.Router) *Worker {
	...
	return &Worker{
		queue:     make(chan entities.Message, 10000),
		publisher: publisher,
		logger:    logging,
		router:    router,
		eventBus:  eventBus,
...
func (w *Worker) Send(msg ...entities.Message) {
	for _, m := range msg {
		w.queue <- m
	}
}
	...
	go func() {
		for msg := range w.queue {
			...
			case entities.Transaction:
				...
				mesg.Metadata.Set("type", "TransactionConfirmed")

				err = w.eventBus.Publish(ctx, mesg)
				if err != nil {
